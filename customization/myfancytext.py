...
_styles = {"normal": wx.FONTSTYLE_NORMAL, "slant": wx.FONTSTYLE_SLANT,
           "italic": wx.FONTSTYLE_ITALIC}
_weights = {"normal": wx.FONTWEIGHT_NORMAL, "light": wx.FONTWEIGHT_LIGHT,
...
_greek_letters = ("alpha", "beta", "gamma", "delta", "epsilon", "zeta",
                  ...
                  "xi", "omnikron", "pi", "rho", "altsigma", "sigma", "tau",
                  "upsilon",
                  "phi", "chi", "psi", "omega")
...
class Renderer:
    ...
    def __init__(self, dc=None, x=0, y=None, line_height_stretch=1.2):
        if dc == None:
            dc = wx.MemoryDC()
        self.dc = dc
        self.offsets = [0]
        ...
        self.line_height_stretch = line_height_stretch
    ...
    def characterData(self, data):
        ...
        for i, chunk in enumerate(data.split('\n')):
            if i:
                self.x = 0
                self.y = self.mayY = self.maxY + self.dc.GetTextExtent("M")[
                    1] * self.line_height_stretch
            if chunk:
                width, height, descent, extl = self.dc.GetFullTextExtent(chunk)
                self.renderCharacterData(data, iround(self.x), iround(
                    self.y + self.offsets[-1] - height + descent))
            else:
    ...
    def start_sub(self, attrs):
        ...
        self.offsets.append(
            self.offsets[-1] + self.dc.GetFullTextExtent("M", font)[1] * 0.5)
    ...
    def start_sup(self, attrs):
        ...
        font = self.getCurrentFont()
        self.offsets.append(
            self.offsets[-1] - self.dc.GetFullTextExtent("M", font)[1] * 0.3)
        self.start_font({"size": font.GetPointSize() * 0.8})
    ...
    def getCurrentColor(self):
        font = self.fonts[-1]
        return wx.TheColourDatabase.FindColour(
            font.get("color", self.defaultColor))
...
class DCRenderer(Renderer):
    ...
    def start_angle(self, attrs):
        ...
        self.dc.DrawLine(iround(self.x), iround(y), iround(self.x + width),
                         iround(y))
        self.dc.DrawLine(iround(self.x), iround(y), iround(self.x + width),
                         iround(y - width))
    ...
    def start_times(self, attrs):
        ...
        self.dc.DrawLine(iround(self.x), iround(y - width),
                         iround(self.x + width - 1), iround(y - 1))
        self.dc.DrawLine(iround(self.x), iround(y - 2),
                         iround(self.x + width - 1), iround(y - width - 1))
        self.updateDims(width, height, 0, 0)
...
def GetFullExtent(str, dc=None, enclose=True):
    renderer = SizeRenderer(dc)
    RenderToRenderer(str, renderer, enclose)
    return iceil(renderer.width), iceil(renderer.height), -iceil(
        renderer.minY)  # XXX round up
...
class StaticFancyText(wx.StaticBitmap):
    def __init__(self, window, id, text, *args, **kargs):
        ...
        if 'background' in kargs:
            ...
            background = wx.Brush(window.GetBackgroundColour(),
                                  wx.BRUSHSTYLE_SOLID)
